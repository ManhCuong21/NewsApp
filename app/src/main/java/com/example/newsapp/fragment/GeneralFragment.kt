...
class GeneralFragment : Fragment() {
    ...
    fun setUpRecycleView() = binding.rcvGeneral.apply {
        newsAdapter = NewsAdapter(context)
        binding.rcvGeneral.adapter = newsAdapter
        binding.rcvGeneral.layoutManager = LinearLayoutManager(context)
    }

    fun callNews(){
        try {
            binding.progressBar.isVisible = true
            binding.rcvGeneral.isVisible = false
            CoroutineScope(Dispatchers.IO).launch {
                ...
                if (response.isSuccessful && response.body() != null) {
                    withContext(Dispatchers.Main){
                        ...
                        binding.rcvGeneral.isVisible = true
                    }
                }
            }
        } catch (e: IOException) {
