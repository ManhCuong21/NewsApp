...
class ScienceFragment : Fragment() {
    ...
    fun setUpRecycleView() = binding.rcvScience.apply {
        newsAdapter = NewsAdapter(context)
        binding.rcvScience.adapter = newsAdapter
        binding.rcvScience.layoutManager = LinearLayoutManager(context)
    }

    fun callNews(){
        try {
            binding.progressBar.isVisible = true
            binding.rcvScience.isVisible = false
            CoroutineScope(Dispatchers.IO).launch {
                ...
                if (response.isSuccessful && response.body() != null) {
                    withContext(Dispatchers.Main){
                        ...
                        binding.rcvScience.isVisible = true
                    }
                }
            }
        } catch (e: IOException) {
